    ...
    for (const dbGuild of dbGuilds) {
      ...
      for (const topic of topics) {
        ...
        const debateMessage = await channel.createMessage(`**${topic.name} debate**\n\n__STATEMENT:__\n${StringUtil.mongoFieldOut(statement).codeBlock()}\n**__REPLY__** with \`agree\` or \`disagree\` to debate!`,
          {footer: {text: `Up to ${Constants.statements.maxDebaters} people with differing opinions will be selected to debate against you along with ${Constants.statements.maxDebaters - 1} people to debate with you.`}});
